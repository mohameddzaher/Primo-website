...
// Search autocomplete (public) - lightweight endpoint for live suggestions
router.get(
  '/search',
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const rawProducts = await Product.find(query)
      .select('title slug price discount images averageRating categoryId brand stockQuantity')
    ...
    const products = rawProducts.map((p: any) => ({
      ...
      category: p.categoryId ? { name: (p.categoryId as any).name, slug: (p.categoryId as any).slug } : null,
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // Auto-create Brand document if this brand name doesn't exist yet
    // This ensures the brand filter on the products page always stays in sync
    if (data.brand) {
      await Brand.findOneAndUpdate(
        { name: data.brand },
        { $setOnInsert: { name: data.brand, isActive: true } },
        { upsert: true }
      );
    }
...
router.patch(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // Auto-create Brand document if a new brand name is being set
