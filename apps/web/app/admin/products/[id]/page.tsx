...
export default function EditProductPage() {
  ...
  useEffect(() => {
    if (product) {
      reset({
        ...
        subcategoryId: product.subcategoryId?._id || (typeof product.subcategoryId === 'string' ? product.subcategoryId : '') || '',
        brand: product.brand || '',
      ...
      // Map Variants. `id` is preserved verbatim — existing cart and order lines
      // reference it, so regenerating it would orphan them.
  ...
  const onSubmit = (data: ProductForm) => {
    ...
    // Only fully-filled variant rows are sent. Sending [] clears all variants and
    // returns the product to plain product-level stock.
    ...
    // Optional string fields — send when present, omit when blank
    for (const key of ['shortDescription', 'deliveryNotes', 'installationNotes', 'metaTitle', 'metaDescription', 'subcategoryId'] as const) {
      const val = (data as any)[key];
      if (val && String(val).trim()) cleanData[key] = String(val).trim();
    }
    if (typeof data.lowStockThreshold === 'number' && !isNaN(data.lowStockThreshold)) {
      cleanData.lowStockThreshold = data.lowStockThreshold;
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {tags.map((tag) => (
                    <span key={tag} className="inline-flex items-center gap-1 px-2.5 py-1 bg-beige-100 text-dark-700 text-sm rounded-full">
                      {tag}
                      <button type="button" onClick={() => removeTag(tag)} className="text-dark-400 hover:text-error-500" title="Remove tag" aria-label={`Remove ${tag}`}>
                        <HiOutlineX size={14} />
                      </button>
                    </span>
                  ))}
          ...
          <div className="space-y-6">
            ...
            <Card padding="lg">
              ...
              <p className="text-xs text-dark-400 mb-4">Optional. Falls back to the product title/description if empty.</p>
              <div className="space-y-4">
                <Input label="Meta Title" placeholder="Up to 60 characters" error={errors.metaTitle?.message} {...register('metaTitle')} />
                <Textarea label="Meta Description" placeholder="Up to 160 characters" rows={3} error={errors.metaDescription?.message} {...register('metaDescription')} />
              </div>
            </Card>
